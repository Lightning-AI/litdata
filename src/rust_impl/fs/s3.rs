...
impl StorageBackend for S3Storage {
    ...
    async fn get_bytes_in_range(
        ...
        let (bucket_name, key): (String, String);

        if _url.starts_with("s3://") == false {
            let _complete_url: String;
            if self.remote_dir.ends_with("/") == false {
                _complete_url = format!("{}/{}", self.remote_dir, _url);
            } else {
                _complete_url = format!("{}{}", self.remote_dir, _url);
            }

            (bucket_name, key) = get_bucket_name_and_key(&_complete_url);
        } else {
            (bucket_name, key) = get_bucket_name_and_key(_url);
        }
