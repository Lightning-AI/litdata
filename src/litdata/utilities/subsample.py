...
def target_sum_problem_with_space_optimization(my_roi_list:List[Tuple[int,int]], target:int)->List[Optional[Tuple[int, ...]]]:
    """
    Solves the target sum problem with space optimization, finding subsets of minimum items that add up to the target.
            ...
            if prev_row[j] is not None:
                ...
                if curr_chunk_size + j <= target:
                    ...
                    if (prev_row[j+curr_chunk_size]) is not None:
                        ...
                        if len(new_probable_tuple) >= len(prev_row_after_curr_chunk_size): # check if the new tuple is better than the previous one (less elements)
...
def my_subsampled_filenames_and_roi(my_chunks: List[Dict[str, Any]], my_roi_list:List[Tuple[int,int]], target:int)->Tuple[List[str],List[Tuple[int, int]], List[Dict[str, Any]], List[Tuple[int, int]]]:
    """
    Selects a subset of filenames and ROIs that best match the target sum, with the remaining items returned separately.
    ...
    while left_item_count>0:
        ...
        if top_left_roi_item_count <= left_item_count:
            ...
            my_subsampled_chunk_filenames.append(top_left_chunk["filename"])
            left_chunks.append(top_left_chunk) # it will also be available for other splits, as not all roi is exhausted
