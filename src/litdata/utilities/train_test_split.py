...
def train_test_split(streaming_dataset: StreamingDataset, splits: List[float], seed: int = 42) -> List[StreamingDataset]:
    ...
    if os.path.exists(os.path.join(input_dir.path, _INDEX_FILENAME)):
        ...
        with open(os.path.join(input_dir.path, _INDEX_FILENAME)) as f:
            ...
            subsampled_chunks = [_org_chunk for _org_chunk in original_chunks if _org_chunk["filename"] in dummy_subsampled_chunk_filename]
    else:
    ...
    for i, split in enumerate(splits):
        ...
        curr_chunk_filename, curr_chunk_roi, left_chunks, left_roi = my_subsampled_filenames_and_roi(subsampled_chunks, dummy_subsampled_roi, item_count)
