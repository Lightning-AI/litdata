...
class LambdaDataChunkRecipe(DataChunkRecipe):
    ...
    def check_fn(self):
        if isinstance(self._fn, partial) or isinstance(self._fn, FunctionType) and inspect.isgeneratorfunction(self._fn) or callable(self._fn) and inspect.isgeneratorfunction(self._fn.__call__):
